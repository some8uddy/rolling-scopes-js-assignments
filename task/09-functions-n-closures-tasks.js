 ...
 */
function getPolynom() {
    let coeffs = [].slice.call(arguments);
    return function (x) {
        let degree = coeffs.length;
        if (degree == 0) return null;
        return coeffs.reduce(function (sum, el, ind) {
            return sum + el * Math.pow(x, degree - 1 - ind)
        }, 0)
    };
}
...
function memoize(func) {
    ...
    return function () {
        if (cashed == undefined) {
            cashed = func.apply(this,arguments);
        }
        return cashed;
    }
}
...
function logger(func, logFunc) {
    ...
    return function () {
        let args=[].slice.call(arguments).map(function(el){
            return checkArgs(el);
        });
        let message = func.name + '(' + args + ')';
